import { mysqlTable, mysqlSchema, AnyMySqlColumn, foreignKey, primaryKey, varchar, int, unique, bigint, timestamp, text, tinyint } from "drizzle-orm/mysql-core"
...
export const desertFigure = mysqlTable("desert_figure", {
	...
	dateAdded: timestamp("date_added", { mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	lastUpdated: timestamp("last_updated", { mode: 'string' }),
	createdBy: varchar("created_by", { length: 255 }).notNull().references(() => user.id, { onDelete: "cascade" } ),
},
(table) => {
	return {
		desertFigureId: primaryKey({ columns: [table.id], name: "desert_figure_id"}),
		fullName: unique("full_name").on(table.fullName),
	}
});

export const excerpt = mysqlTable("excerpt", {
	id: bigint("id", { mode: "number" }).autoincrement().notNull(),
...
export const excerptTag = mysqlTable("excerpt_tag", {
	excerptId: bigint("excerpt_id", { mode: "number" }).notNull(),
	tagId: bigint("tag_id", { mode: "number" }).notNull(),
},
(table) => {
	return {
		excerptTagExcerptIdTagId: primaryKey({ columns: [table.excerptId, table.tagId], name: "excerpt_tag_excerpt_id_tag_id"}),
	}
});

export const icon = mysqlTable("icon", {
	id: bigint("id", { mode: "number" }).autoincrement().notNull(),
